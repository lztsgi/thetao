 ...
 *
 * ——【本次增强】——
 * [tags=流媒体/AI[/IPv6...]] 仅检测并保留这里列出的关键词；用 / 或 | 或 + 分隔。
 *   - 英文关键词大小写不敏感（并使用词边界），中文/混合按子串匹配。
 *   - 例如：#tags=流媒体/AI/IPv6
 */
...
const TAG_KEYS = TAGS
  ...
  : [];
function makeKwReg(kw) {
  const escaped = kw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  if (/^[A-Za-z]+$/.test(kw)) {
    return new RegExp(`\\b${escaped}\\b`, 'i'); // 英文：词边界+不区分大小写
  }
  return new RegExp(escaped); // 中文/混合：子串匹配
}
const TAG_RES = TAG_KEYS.map(makeKwReg);
...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    if (TAG_RES.length > 0) {
      for (let i = 0; i < TAG_RES.length; i++) {
        if (TAG_RES[i].test(originalName)) {
          e.__tags.push(TAG_KEYS[i]); // 使用用户填入的原文字样
        }
      }
    }
  });
  ...
  // 变更点④（动态）：在编号之后统一追加后缀（-关键词1|关键词2）
  pro.forEach((e) => {
    if (Array.isArray(e.__tags) && e.__tags.length > 0) {
      e.name = `${e.name}-${e.__tags.join('|')}`;
    }
    delete e.__tags; // 清理临时属性
  });
